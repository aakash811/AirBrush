...
MongoClient.connect(connectionString, (err, client) => {
  ...
  app.get("/blog/:url", async (req, res) => {
    try {
      ...
      article.date = article.date.toISOString().split('T')[0];
      const relatedArticles = await blogCollection.aggregate([
    ...
    try {
      if (conversation.length === 1) {
        ...
        if (getTotalWordCount([singleMessage]) > wordLimit) {
          conversation[0] = await truncateMessageToWordLimit(singleMessage, wordLimit);
        }
      } else {
  ...
  async function aiChat(conversation) {
    ...
    while (attempts < maxRetries) {
      try {
        const response = await fetch("https://app.ai4chat.co/api/v1/chat/completions", requestOptions);
        ...
        if (attempts >= maxRetries) {
          console.error("Function calling failed after multiple attempts:", error);
  ...
  app.get("/character/:url", async (req, res) => {
    characterCollection.findOne({ url: req.params.url }, async (err, character) => {
      if (!character) {
        return res.redirect('/404');
      } else {
        res.render('generator-character', {
          ...
          faq: character.faq || '',
          conversationStarters: Array.isArray(character.conversationStarters) ? character.conversationStarters : []
        });
  ...
  async function chatgpt3(conversation, aiengine) {
    ...
    try {
      ...
      assistantMessage = "There was an error processing your request. Please try again.";
      console.log("Assistant's response:", assistantMessage);
      throw error;
    }
  }
